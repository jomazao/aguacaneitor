                    ...
                    else
                    {
                        ...
                        while (1)
                        {
                            if(currentCycle >= sendCycle) {
                                float Temp;
                                float Humi;
                                float niv=(analogRead(0))*100/1024;
                                ...
                                thermostat->nivel=niv;
                                currentCycle = 0;
                                unsigned char*buffer;
                                size_t bufferSize;

                                LogInfo("Enviando los valores sensados Temperatura = %d, Humedad = %d, Nivel = %d\r\n", thermostat->Temperature, thermostat->Humidity,thermostat->nivel);

                                if (SERIALIZE(&buffer, &bufferSize, thermostat->DeviceId, thermostat->Temperature, thermostat->Humidity, thermostat->ExternalTemperature,thermostat->nivel) != IOT_AGENT_OK)
                                {
